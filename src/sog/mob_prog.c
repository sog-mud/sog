	...
	const void *arg1, const void *arg2, CHAR_DATA *rch)
{
	...
	{
	    log(LOG_INFO, "cmd_eval: vnum %d: syntax error(2) '%s'",
		vnum, original);
	    return FALSE;
	}
    ...
    if (buf[0] != '$' || buf[1] == '\0')
    {
	log(LOG_INFO, "cmd_eval: vnum %d: syntax error(3) '%s'",
		vnum, original);
        return FALSE;
    }
    else
	...
	default:
	    log(LOG_INFO, "cmd_eval: vnum %d: syntax error(4) '%s'",
		vnum, original);
	    return FALSE;
    }
    /*
	...
	case CHK_SAMECLAN:
		return (lval_char && IS_CLAN(lval_char->clan, mob->clan));
	default:;
     }
	...
	case CHK_CLAN:
	    return(lval_char != NULL && IS_CLAN(buf, lval_char->clan));
	case CHK_RACE:
	    return(lval_char != NULL && IS_RACE(lval_char->race, buf));
	case CHK_CLASS:
    ...
    if ((oper = keyword_lookup(fn_evals, buf)) < 0)
    {
	log(LOG_INFO, "cmd_eval: vnum %d: syntax error(5): '%s'",
		vnum, original);
	return FALSE;
    }
    one_argument(line, buf, sizeof(buf));
	...
	case CHK_OBJVAL0:
            if (lval_obj != NULL) lval = INT(lval_obj->value[0]); break;
        case CHK_OBJVAL1:
            if (lval_obj != NULL) lval = INT(lval_obj->value[1]); break;
        case CHK_OBJVAL2: 
            if (lval_obj != NULL) lval = INT(lval_obj->value[2]); break;
        case CHK_OBJVAL3:
            if (lval_obj != NULL) lval = INT(lval_obj->value[3]); break;
	case CHK_OBJVAL4:
	    if (lval_obj != NULL) lval = INT(lval_obj->value[4]); break;
	case CHK_GRPSIZE:
		  ...
		  const void *arg1, const void *arg2)
{
	...
	if ((mprog = mpcode_lookup(pvnum)) == NULL) {
		log(LOG_INFO, "program_flow: mob vnum %d: mprog vnum %d: not found",
		    mob->pMobIndex->vnum, pvnum);
		return;
	}

	if (++call_level > MAX_CALL_LEVEL) {
		log(LOG_INFO, "program_flow: vnum %d: MAX_CALL_LEVEL exceeded",
		pvnum);
		goto bail_out;
	}
	...
	*b = *c = *d = '\0';
	...
	if (!str_cmp(control, "if"))
	{
	    ...
	    {
		log(LOG_INFO, "program_flow: vnum %d: misplaced if statement",
			pvnum);
		goto bail_out;
	    }
	    state[level] = BEGIN_BLOCK;
            ...
            {
		log(LOG_INFO, "program_flow: vnum %d: max nested level exceeded",
			pvnum);
		goto bail_out;
	    }
	    ...
	    {
		log(LOG_INFO, "program_flow: vnum %d: invalid if_check (if)",
			pvnum);
		goto bail_out;
	    }
	...
	else if (!str_cmp(control, "or"))
	{
	    ...
	    {
		log(LOG_INFO, "program_flow: vnum %d: 'or' without 'if'",
			pvnum);
		goto bail_out;
	    }
		...
		eval = cmd_eval(pvnum, line, check, mob, ch, arg1, arg2, rch);
            ...
            {
		log(LOG_INFO, "program_flow: vnum %d: invalid if_check (or)",
			pvnum);
		goto bail_out;
            }
	...
	else if (!str_cmp(control, "and"))
	{
	    if (!level || state[level-1] != BEGIN_BLOCK)
	    {
		log(LOG_INFO, "program_flow: vnum %d: 'and' without 'if'",
			pvnum);
		goto bail_out;
	    }
	    if (level && cond[level-1] == FALSE) continue;
	    ...
	    {
		log(LOG_INFO, "program_flow: vnum %d: invalid if_check (and)",
			pvnum);
		goto bail_out;
	    }
	...
	else if (!str_cmp(control, "endif"))
	{
	    if (!level || state[level-1] != BEGIN_BLOCK)
	    {
		log(LOG_INFO, "program_flow: vnum %d: 'endif' without 'if'",
			pvnum);
		goto bail_out;
	    }
	    cond[level] = TRUE;
	...
	else if (!str_cmp(control, "else"))
	{
	    if (!level || state[level-1] != BEGIN_BLOCK)
	    {
		log(LOG_INFO, "program_flow: vnum %d: 'else' without 'if'",
			pvnum);
		goto bail_out;
	    }
	    if (level && cond[level-1] == FALSE) continue;
