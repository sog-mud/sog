...
int     imc_debug_on;
int     imc_minplayerlevel;	/* Minumum level IMC will see players at, */
				/* defaults to 10 */
int     imc_minimmlevel;	/* Minimum level IMC sees someone as an */
				/* immortal, defaults to IMCMAX_LEVEL	*/
int     imc_minadminlevel;	/* Minimum level IMC sees someone as an	*/
				/* administrator, defaults to IMCMAX_LEVEL */
...
bool
imc_str_prefix(const char *astr, const char *bstr)
{
	if (!astr || astr[0] == '\0') {
...
bool
level_pass(CHAR_DATA * ch, unsigned char perm_level)
{
	int     level;
...
/*
 * Finds the permission level for a player using level_pass
 * and returns the permission level
 */
unsigned char
get_permlevel(CHAR_DATA * ch)
{
...
/*
 * Checks if a network function is able to be used by a player by checking
 * deny and allow flags, followed by a permission level check - preceeding
 * all these checks is a non-npc check
 */
bool
function_usable(CHAR_DATA * ch, long deny_flag, long allow_flag,
...
/*
 * Generic log function which will route the log messages to the appropriate
...
/*
 * Generic bug logging function which will route the message to the appropriate
 * function that handles bug logs
 */
void
...
/*
 * Table is formatted with mud color code first, then ANSI code,
 * then IMC color code.
 * This should make it easier to modify local color codes.
 *
 * You need to change the &, } and { tokens used in the table below
...
imccolor_struct imccolor_table[] = {
	...
	/*
	 * First three characters in second column are used for mud color
	 * parsing tests.
	 * ~ is the color token used in the IMC mud standard.
	 * All {, }, and & must be replaced with your mud's color tokens
	 * to convert the table to your mud effectively. 'Doubling up' in
	 * this table is to escape color codes. If your mud does not do this,
	 * then make these single characters. Most muds do escape.
	 */
	{ "&&", "&", "&" },
	{ "{{", "{", "{" },
	{ "}}", "}", "}" },
	...
	{ "{b", "", "~b" },	/* Dark Blue		*/
	{ "{m", "", "~p" },	/* Purple/Magenta	*/
	{ "{m", "", "~m" },	/* Duplicated to support duplicate	*/
				/* IMC code for color			*/
	{ "{c", "", "~c" },	/* Cyan			*/
	{ "{w", "", "~w" },	/* Grey			*/
	{ "{w", "", "~d" },	/* Duplicated to support triplicate	*/
				/* IMC code for color			*/
	{ "{x", "", "~!" },	/* Triplicated to support IMC reset	*/
				/* code for color (defaults to grey)	*/
...
color_itom(const char *s)
{
	...
	for (current = s, out = buf, count = 0;
	    *current && count < IMC_DATA_LENGTH;) {
		...
		if (c != 4) {
			for (i = 0; imccolor_table[i].mud != NULL; i++) {
				...
				if (l
				&& !strncmp(current, imccolor_table[i].imc, l))
					break;
			}
			...
			if (imccolor_table[i].mud != NULL) {	/* match */
				...
				if (count >= IMC_DATA_LENGTH)
					break;
				strlcpy(out, imccolor_table[i].mud,
				    IMC_DATA_LENGTH);
				out += len;
				current += l;
				continue;
	    ...
	    *current && count < IMC_DATA_LENGTH;) {
		...
		if (c != 4) {
			for (i = 0; imccolor_table[i].mud != NULL; i++) {
				...
				if (l
				&& !strncmp(current, imccolor_table[i].mud, l))
					break;
			}
			...
			if (imccolor_table[i].mud != NULL) {	/* match */
				...
				if (count >= IMC_DATA_LENGTH)
					break;
				strlcpy(out, imccolor_table[i].imc,
				    IMC_DATA_LENGTH);
				out += len;
				current += l;
				continue;
...
int
imc_strlen_color(const char *str)
{
	int     c, i, length;
...
char *
EmptyBlock(const char *argument, int leftsidespacing, int rightsidespacing)
{
	static char buf[LSS];
	int     length =
	    imc_strlen_color(argument) + leftsidespacing + rightsidespacing;
	int     i = 0;
...
/*
 * Inserts an alternating pattern of two symbols(possibly colors)
 * into any text
 */
char *
Alternating_Symbols(const char *argument, const char *symbol1,
    const char *symbol2)
{
	static char buf[LSS];
	bool    start = TRUE;
	...
	if (!argument || argument[0] == '\0' || !symbol1 || symbol1[0] == '\0'
	|| !symbol2 || symbol2[0] == '\0')
		return NULL;
...
char *
SetFill(const char *argument, int size, const char *filler)
{
...
/*
 * Generic send_to_pager type function to send to the proper code
 * for each codebase
...
void
imc_add_event(int when, void (*callback) (void *), void *data, int timed)
{
...
void
imc_cancel_event(void (*callback) (void *), void *data)
{
...
const char *
imc_getkey(PACKET * p, const char *key, const char *def)
{
	int     i;
...
int
imc_getkeyi(PACKET * p, char *key, int def)
{
	int     i;
...
/* add "key=value" to "p" */
void
imc_addkey(PACKET * p, const char *key, const char *value)
{
	int     i;
...
void
imc_addkeyi(PACKET * p, const char *key, int value)
{
	char    temp[20];
...
/* clone packet data */
void
imc_clonedata(PACKET * p, PACKET * n)
{
	int     i;
...
int
checkrepeat(const char *mud, unsigned long seq)
{
	int     i;

	for (i = 0; i < IMC_MEMORY; i++)
		if (imc_memory[i].from && !strcasecmp(mud, imc_memory[i].from)
...
const char *
imc_nameof(const char *fullname)
{
...
char *
imc_makename(const char *player, const char *mud)
{
 ...
 */
char *
imc_getarg(const char *argument, char *arg, int length)
{
	static char buf[LSS];
	int     len = 0;
...
int
imc_hasname(const char *list, const char *name)
{
	char   *p;
...
void
imc_addname(const char **list, char *name)
{
...
void
imc_removename(const char **list, char *name)
{
	char    buf[1000];
	char    arg[IMC_NAME_LENGTH];
	char   *p;
...
printkeys(PACKET * data)
{
	...
	for (i = 0; i < IMC_MAX_KEYS; i++) {
		...
		if (!strchr(data->value[i], ' '))
			strlcpy(temp, escape2(data->value[i]),
			    IMC_DATA_LENGTH - 1);
		else {
			temp[0] = '"';
			strlcpy(temp + 1, escape2(data->value[i]),
			    IMC_DATA_LENGTH - 3);
...
void
parsekeys(const char *string, PACKET * data)
{
	const char *p1;
	char   *p2;
...
generate2(PACKET * p)
{
	...
	if (!p->i.path[0])
		...
		snprintf(newpath, IMC_PATH_LENGTH, "%s!%s", p->i.path,
		    imc_name);
...
interpret2(char *argument)
{
	...
	if (!out.i.from[0] || !seq[0] || !out.i.path[0] || !out.type[0]
	|| !out.i.to[0]) {
...
inpath(char *path, char *name)
{
	...
	&& !strcasecmp(tempp + strlen(tempp) - strlen(buf), buf))
		return 1;
...
REMOTEINFO *
imc_find_reminfo(const char *name, int type)
{
	REMOTEINFO *p;
...
void
receive_oldchannel(imc_char_data * from, int number, const char *argument,
    int emote)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *victim;
	...
	if (number == 2) {
		...
		if (!emote)
			snprintf(buf, IMC_DATA_LENGTH,
			    "~c[~CIMC-Status~c] ~C%s ~cannounces '%s'\n",
			    from->name, argument);
		else
			snprintf(buf, IMC_DATA_LENGTH,
			    "~c[~CIMC-Status~c] ~C%s ~c%s\n", from->name,
			    argument);
	} else {
		chan = 1;
		...
		if (!emote)
			snprintf(buf, IMC_DATA_LENGTH,
			    "~c[~CIMC-Chan-FYI~c] ~C%s ~cannounces '%s'\n",
			    from->name, argument);
		else
			snprintf(buf, IMC_DATA_LENGTH,
			    "~c[~CIMC-Chan-FYI~c] ~C%s ~c%s\n", from->name,
			    argument);
	}
	...
	for (d = first_descriptor; d; d = d->next) {
		if (d->connected == CON_PLAYING
		    && (victim =
		    d->original ? d->original : d->character) != NULL
		    && !IS_SET(IMC_PFLAGS(victim),
		    imc_functions[chan].pset_flag)
		    && function_usable(victim, imc_functions[chan].deny_flag,
		    imc_functions[chan].allow_flag,
...
void
imc_recv_chat(imc_char_data * from, int channel, const char *argument)
{
	receive_oldchannel(from, channel, argument, 0);
}

void
imc_recv_emote(imc_char_data * from, int channel, const char *argument)
{
...
void
setdata(PACKET * p, imc_char_data * d)
{
	imc_initdata(p);
...
void
imc_send_tell(imc_char_data * from, char *to, const char *argument, int isreply)
{
	PACKET  out;
...
void
imc_sendblacklistresponse(char *to)
{
	char    buf[IMC_DATA_LENGTH];

	if (strcasecmp(imc_nameof(to), "*")) {
		snprintf(buf, IMC_DATA_LENGTH,
		    "%s is blocking packets from your mud.", imc_name);
		imc_send_tell(NULL, to, buf, 1);
	}
}

void
imc_recv_imcptell(imc_char_data * from, char *to, const char *argument,
    int isreply)
{
	DESCRIPTOR_DATA *d;
	...
	for (entry = FIRST_IMCBLACKLIST(victim); entry; entry = entry->next)
		if (!strcasecmp(from->name, entry->name)
		|| !strcasecmp(imc_mudof(from->name), entry->name))
			break;
...
void
imc_recv_whoreply(const char *to, const char *text, int sequence, int inlen)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *victim;
...
const char *
channel_mudof(const char *fullname)
{
	static char buf[IMC_PNAME_LENGTH];
	char   *where = buf;
	int     count = 0;
...
bool
imc_audible(IMC_CHANNEL * c, const char *who)
{
	...
	if (!strchr(who, '@')) {
		...
		/*
		 * Private channel - can only hear if someone with
		 * the right mud name is there.
		 */
	...
	if (!strcasecmp(imc_nameof(who), "IMC")
	...
	if (c->policy == CHAN_OPEN || c->policy == CHAN_COPEN) {
		/*
		 * open policy. default yes. override with excludes,
		 * then invites
		 */
	...
	/*
	 * closed or private. default no, override with invites,
	 * then excludes
	 */
...
void
update_imchistory(IMC_CHANNEL * channel, const char *message)
{
...
void
imc_showchannel(IMC_CHANNEL * c, const char *from, const char *txt, int emote)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *ch;
	char    buf[LSS];
	...
	if (emote < 2)
		snprintf(buf, LSS, emote ? c->emoteformat : c->regformat, from,
		    color_itom(txt));
	else
...
void
imc_sendmessage(IMC_CHANNEL * c, const char *name, const char *text, int emote)
{
	PACKET  out;
	...
	if (c->policy == CHAN_PRIVATE || c->policy == CHAN_CPRIVATE) {
		...
		snprintf(out.to, IMC_NAME_LENGTH, "IMC@%s",
		    channel_mudof(c->name));
	} else {
		/* broadcast */
...
void
imc_send_whoreply(const char *to, const char *data, int sequence)
{
	PACKET  out;
...
void
imc_whoreply_start(const char *to)
{
	wr_sequence = 0;
...
void
imc_whoreply_add(const char *text)
{
...
imcrankbuffer(CHAR_DATA * ch)
{
	...
	if (CH_IMCRANK(ch) && CH_IMCRANK(ch)[0] != '\0') {
		snprintf(rbuf, LSS, "~%c%s", rankcolor_code[perm_level],
		    color_mtoi(CH_IMCRANK(ch)));
		return rbuf;
	}
	...
	if (perm_level < PERM_IMMORTAL)
		...
		snprintf(rbuf, LSS, "~%cAdministrator",
		    rankcolor_code[perm_level]);
	else
 ...
 */
void
process_imcplist(imc_char_data * from, const char *argument)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *person;
	...
	case 1:		/* The Winter Who Style by Kris */
		...
		imc_whoreply_add("\n\r");
		snprintf(buf2, LSS, "%s~C|", Alternating_Symbols(buf, "~W",
		    "~C"));
		imc_whoreply_add("~C|");
		...
		if (imc_siteinfo.port > 0)
			...
			snprintf(buf, LSS, "~W**** ~wtelnet://%s ~W****",
			    imc_siteinfo.host);
	...
	case 2:		/* The Fiery Bowels of Hell Who Style by Kris */
		...
		imc_whoreply_add("\n\r");
		imc_whoreply_add(SetFill
		    ("~R (~r' ~R)~r,~R)~r.  ~B_~R)~B_  ~R( (~r,~R)~r' ~R)  (~r"
		     "' ~R)~r,~R)~r.  ~B_~R)~B_  ~R( (~r,~R)~r' ~R) (~r' ~R)~r"
		     ",~R)~r.  ~B_~R)~B_  ~R( (~r,~R)~r' ~R)  (~r' ~R)~r,~R)~r"
		     ". ~R", 78, " "));
		imc_whoreply_add("\n\r");
		...
		if (imc_siteinfo.port > 0)
			...
			snprintf(buf, LSS, "~Y--->  ~ytelnet://%s  ~Y<---",
			    imc_siteinfo.host);
	...
	case 3:		/*
			 * IMC2 Classic Who Style - Slight altered from
			 * older versions of the IMC2 client
			 */
		snprintf(buf, LSS, "~R-=[ ~WPlayers on %s ~R]=-",
		    imc_siteinfo.name);
		imc_whoreply_add(SetFill(buf, 78, " "));
		imc_whoreply_add("\n\r");

		if (imc_siteinfo.port > 0)
			snprintf(buf, LSS, "~Y-=[ ~Wtelnet://%s:%d ~Y]=-",
			    imc_siteinfo.host, imc_siteinfo.port);
		else
			snprintf(buf, LSS, "~Y-=[ telnet://%s ]=-",
			    imc_siteinfo.host);
	...
	default:	/* IMC2 4.00 Continuum Default Who Style by Kris */
		...
		imc_whoreply_add("\n\r");
		snprintf(buf2, LSS, "%s~G|", Alternating_Symbols(buf, "~Y",
		    "~C"));
		imc_whoreply_add("~G|");
		...
		if (imc_siteinfo.port > 0)
			...
			snprintf(buf, LSS, "~C**** ~ctelnet://%s ~C****",
			    imc_siteinfo.host);
	...
	/*
	 * See if there are any visible immortals online
	 * to even display players header.
	 */
	for (d = first_descriptor; d; d = d->next)
		if (d->character && (d->connected == CON_PLAYING)
		&& level_pass(d->character, PERM_IMMORTAL)
	...
	if (i > 0) {
		switch (imc_whostyle) {
		case 1:	/* The Winter Who Style by Kris */
			imc_whoreply_add(SetFill("~CAdmins  ~Wand  ~CImmortals",
			    78, " "));
			imc_whoreply_add("\n\r~W");
		...
		case 2:	/* The Fiery Bowels of Hell Who Style by Kris */
			imc_whoreply_add(SetFill
			    ("~RAdmins~Y----~rand~Y----~RImmortals", 78,
			    "~Y-"));
			break;
		case 3:	/*
			 * IMC2 Classic Who Style - Slight altered
			 * from older versions of the IMC2 client
			 */
			imc_whoreply_add(SetFill("-=[ ~WImmortals ~R]=-", 78,
			    "~R-"));
			break;
		default: /* IMC2 4.00 Continuum Default Who Style by Kris */
			imc_whoreply_add(SetFill("~CAdmins  ~cand  ~CImmortals",
			    78, " "));
			imc_whoreply_add("\n\r~Y");
			imc_whoreply_add(SetFill("", 78, "-"));
			break;
		...
		for (d = first_descriptor; d; d = d->next) {
			if (d->character && d->connected == CON_PLAYING) {
				...
				if (!level_pass(d->character, PERM_IMMORTAL)
				|| !IS_IMCVISIBLE(person))
					continue;

				pcount++;

				strlcpy(rank, SetFill(imcrankbuffer(person), 22,
				    " "), LSS);
				...
				switch (imc_whostyle) {
				case 2:	/*
					 * The Fiery Bowels of Hell Who
					 * Style by Kris
					 */
					snprintf(personbuf, LSS,
					    "~Y%s ~y%s ~Y%s~y%s\n\r", rank,
					    stats, CH_IMCNAME(person),
					    color_mtoi(CH_IMCTITLE(person)));
					break;
				case 3:	/*
					 * IMC2 Classic Who Style - Slight
					 * altered from older versions of
					 * the IMC2 client
					 */
					snprintf(personbuf, LSS,
					    "%s ~D%s ~G%s%s\n\r", rank, stats,
	...
	/*
	 * See if there are any visible players online to even display
	 * players header.
	 */
	for (d = first_descriptor; d; d = d->next)
		if (d->character && (d->connected == CON_PLAYING)
	...
	if (i > 0) {
		...
		case 1:	/* The Winter Who Style by Kris */
			imc_whoreply_add("\n\r\n\r");
			imc_whoreply_add(SetFill("  ~GMortal Players  ", 78,
			    "~W*"));
			break;
		case 2:	/* The Fiery Bowels of Hell Who Style by Kris */
			imc_whoreply_add("\n\r\n\r");
			imc_whoreply_add(SetFill("  ~RMortal Players  ", 78,
			    "~Y*"));
			break;
		case 3:	/*
			 * IMC2 Classic Who Style - Slight altered from
			 * older versions of the IMC2 client
			 */
			imc_whoreply_add(SetFill("-=[ ~WPlayers ~B]=-", 78,
			    "~B-"));
			break;
		default:/* IMC2 4.00 Continuum Default Who Style by Kris */
			imc_whoreply_add("\n\r\n\r");
	...
	case 1:	/* The Winter Who Style by Kris */
		...
		snprintf(tailbuf, LSS,
		    "~W** ~GHomepage: ~g%s ~W**-** ~C%3d ~cMax Since Reboot ~W**\n\r",
	...
	case 2:	/* The Fiery Bowels of Hell Who Style by Kris */
		...
		snprintf(tailbuf, LSS,
		    "~R** ~YHomepage: ~y%s ~R**-** ~Y%3d ~yMax Since Reboot ~R**\n\r",
		    imc_siteinfo.www, IMCMAXPLAYERS);
		imc_whoreply_add(tailbuf);
		break;
	case 3:	/*
		 * IMC2 Classic Who Style - Slight altered from older
		 * versions of the IMC2 client
		 */
		snprintf(tailbuf, LSS, "\n\r~Y[~W%d Player%s~Y] ", pcount,
		    pcount == 1 ? "" : "s");
		imc_whoreply_add(tailbuf);

		snprintf(tailbuf, LSS,
		    "~Y[~WHomepage: %s~Y] [~W%3d Max Since Reboot~Y]\n\r",
		    imc_siteinfo.www, IMCMAXPLAYERS);
		imc_whoreply_add(tailbuf);
		break;
	default:/* IMC2 4.00 Continuum Default Who Style by Kris */
		...
		snprintf(tailbuf, LSS,
		    "~Y** ~GHomepage: ~g%s ~Y**-** ~C%3d ~cMax Since Reboot ~Y**\n\r",
		    imc_siteinfo.www, IMCMAXPLAYERS);
		imc_whoreply_add(tailbuf);
		break;
	}
...
void
process_imcpinfo(imc_char_data * from, const char *to)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *victim = NULL;
	...
	if (!d) {
		imc_send_whoreply(from->name,
		    "That player is either currently offline or not visible.\n\r",
		    -1);
		return;
	}
	...
	for (i = 0; i < numfunctions; i++) {
		if (function_usable(victim, imc_functions[i].deny_flag,
		    imc_functions[i].allow_flag, imc_functions[i].minlevel)
		&& !IS_SET(IMC_PFLAGS(victim), imc_functions[i].pset_flag))
			snprintf(functions + strlen(functions), LSS, "%s ",
	...
	snprintf(buf, LSS, "~cPlayer Information\n\r"
	    ...
	    CH_IMCNAME(victim), male_female[URANGE(0, CH_IMCSEX(victim), 2)],
	    permissions, (IMC_CSUBSCRIBED(victim)
	    && IMC_CSUBSCRIBED(victim)[0] !=
	    '\0' ? IMC_CSUBSCRIBED(victim) : "None"), (functions
	    && functions[0] != '\0' ? functions : ""));
...
imc_connect_to(void)
{
	...
	snprintf(buf, IMC_DATA_LENGTH, "PW %s %s version=%d %s", imc_name,
	    this_imcmud->clientpw, IMC_VERSION, imc_siteinfo.netname);
	do_imcsend(buf);
...
imc_list(void)
{
	...
	snprintf(buf, IMC_DATA_LENGTH, "~WActive muds on IMC:\n"
	    ...
	    imc_siteinfo.name, imc_versionid, imc_siteinfo.netname,
	    this_imcmud->hubname);

	for (p = first_rinfo; p; p = p->next, count++) {
		snprintf(buf + strlen(buf), IMC_DATA_LENGTH - strlen(buf),
		    "\n~c%-15.15s  ~C%-35.35s  ~g%-10.10s  ~G%-10.10s",
		    p->name, p->version, p->netname,
		    p->expired ? "~Rexpired" : imc_hubinpath(p->path));
	}
...
const char *
imc_getstats(const char *choice)
{
	static char buf[IMC_DATA_LENGTH];
	int     evcount = 0;
	...
	if (!choice || choice[0] == '\0' || !strcasecmp(choice, "network")) {
		...
		snprintf(buf, IMC_DATA_LENGTH,
		    ...
		    "~cDebug              : ~C%s\n"
		    "~cLast IMC Boot      : ~C%s\n",
		    imc_stats.rx_pkts, comma(imc_stats.rx_bytes),
		    imc_stats.rx_bytes / secondspast, imc_stats.tx_pkts,
		    comma(imc_stats.tx_bytes), imc_stats.tx_bytes / secondspast,
		    imc_stats.max_pkt, evcount, imc_stats.sequence_drops,
		    imc_debug_on ? "Yes" : "No", ctime(&imc_boot));
	...
	if (!strcasecmp(choice, "general")) {
		snprintf(buf, IMC_DATA_LENGTH,
		    ...
		    "~cAdmin Email    ~W:~C %s\n"
		    "~cDetails        ~W:~C %s\n",
		    imc_siteinfo.name, imc_versionid, imc_siteinfo.host,
		    imc_siteinfo.port, imc_siteinfo.www, imc_siteinfo.email,
...
void
imc_recv_who(imc_char_data * from, const char *type)
{
	char    arg[LSS];
	...
	else if (!strcasecmp(arg, "options") || !strcasecmp(arg, "services")
	    || !strcasecmp(arg, "help"))
		imc_send_whoreply(from->name,
...
void
imc_send_whoisreply(const char *to, const char *data)
{
	PACKET  out;
...
void
imc_recv_whois(imc_char_data * from, const char *to)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *victim;
	char    buf[LSS];
	...
	/*
	 * This circumvents the Validator packets that were used to deter
	 * people from being in an invisible state from IMC2 3.00 to 3.10.
	 * They were easily circumvented and so removed from the IMC2 client
	 * code.
	 */
	...
	for (d = first_descriptor; d; d = d->next) {
		...
		&& (!strcasecmp(to, CH_IMCNAME(victim))
		    || (strlen(to) > 3
			&& !imc_str_prefix(to, CH_IMCNAME(victim))))) {
			snprintf(buf, LSS, "imcpfind %s : %s@%s is online.\n\r",
			    to, CH_IMCNAME(victim), imc_name);
			imc_send_whoisreply(from->name, buf);
		}
	}
}
...
void
imc_recv_whoisreply(const char *to, const char *text)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *victim;
...
void
imc_recv_imcpbeep(imc_char_data * from, const char *to)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *victim = NULL;
	IMC_BLACKLIST *entry;
...
void
imc_recv_keepalive(const char *from, const char *version,
    const char *networkname)
{
	...
	/*
	 * this should never fail, imc.c should create an entry
	 * if one doesn't exist (in the path update code)
	 */
	...
	if (strcasecmp(version, p->version)) {
...
void
imc_send_traceroute(const char *to, const char *playername)
{
	PACKET  out;
...
void
imc_send_traceroutereply(const char *to, const char *path)
{
	PACKET  out;
...
void
imc_recv_traceroute(const char *from, const char *path)
{
...
const char *
display_route(const char *route)
{
	static char buf[LSS];
	int     i = 0;
...
void
imc_recv_traceroutereply(const char *from, const char *to, const char *pathto,
    const char *pathfrom)
{
	...
	for (d = first_descriptor; d; d = d->next)
		...
		&& (!strcasecmp(CH_IMCNAME(ch), pinger)
		|| !strcasecmp(CH_IMCNAME(ch), to)))
			break;
	...
	if (pathfrom && pathfrom[0] != '\0')
		snprintf(incoming, LSS, "%s->%s", display_route(pathfrom),
		    imc_name);

	if (pathto && pathto[0] != '\0')
		snprintf(outgoing, LSS, "%s->%s", display_route(pathto), from);

	char_printf(ch, color_itom("~WTraceroute Information for %s\n"
	    "~cSend Path    ~W:   ~C%s\n"
...
void
getdata(PACKET * p, imc_char_data * d)
{
...
imc_recv(PACKET * p)
{
	...
	if (!strcasecmp(p->type, "chat") && !imc_isblacklisted(p->from))
		imc_recv_chat(&d, imc_getkeyi(p, "channel", 0), imc_getkey(p,
		    "text", ""));

	/* emote: emote to a channel (broadcast) */
	else if (!strcasecmp(p->type, "emote") && !imc_isblacklisted(p->from))
		imc_recv_emote(&d, imc_getkeyi(p, "channel", 0), imc_getkey(p,
		    "text", ""));
	...
	/* tell: tell a player here something */
		...
		if (imc_isblacklisted(p->from))
			...
			imc_recv_imcptell(&d, p->to, imc_getkey(p, "text", ""),
			    imc_getkeyi(p, "isreply", 0));
	}

	/* who-reply: receive a who response */
	else if (!strcasecmp(p->type, "who-reply")
	    || !strcasecmp(p->type, "wHo-reply"))
		imc_recv_whoreply(p->to, imc_getkey(p, "text", ""),
		    imc_getkeyi(p, "sequence", -1), -1);
	...
	/* is-alive: receive a keepalive (broadcast) */
		...
		imc_recv_keepalive(imc_mudof(p->from), imc_getkey(p,
		    "versionid", "unknown"), imc_getkey(p, "networkname",
		    "unknown"));
	...
	/* traceroute-reply: receive a send path and a return path */
		...
		imc_recv_traceroutereply(imc_mudof(p->from), imc_nameof(p->to),
		    imc_getkey(p, "path", NULL), p->i.path);
	...
	/* expire closed hubs - shogar */
		...
		if (r) {
			...
			for (r = first_rinfo; r; r = r->next) {
				...
				snprintf(fake, 90, "!%s", imc_getkey(p, "host",
				    "___unknown "));
				if (r->name && r->path
				&& (sf = strstr(r->path, fake)) && sf
				&& (*(sf + strlen(fake)) == '!'
				|| *(sf + strlen(fake)) == 0)) {
					r->expired = TRUE;
...
imc_send(PACKET * p)
{
	...
	strlcpy(p->i.from + strlen(p->i.from), imc_name,
...
void
ev_request_keepalive(void *data)
{
...
do_imcread(void)
{
	...
	if (!r || (r < 0 && errno != EAGAIN && errno != EWOULDBLOCK)) {
		if (r < 0)	/* read error */
			imclog("Read error on connection to %s",
			    this_imcmud->hubname);
		else		/* socket was closed */
			imclog("EOF encountered on connection to %s",
			    this_imcmud->hubname);
...
do_imcwrite(void)
{
	...
	if (!w || (w < 0 && errno != EAGAIN && errno != EWOULDBLOCK)) {
		if (w < 0)	/* write error */
			imcbug("Write error on connection to %s",
			    this_imcmud->hubname);
		else		/* socket was closed */
			imcbug("EOF encountered on connection to %s",
			    this_imcmud->hubname);
	...
	strlcpy(this_imcmud->outbuf, this_imcmud->outbuf + w,
	    this_imcmud->outsize);
...
/*
 * handle a password response from a server - for connection negotiation.
 * DON'T REMOVE THIS
 */
void
serverpassword(char *argument)
{
	...
	if (!imc_vinfo[rversion].generate || !imc_vinfo[rversion].interpret) {
		imclog("%s: Unsupported version %d", this_imcmud->hubname,
		    rversion);
		imc_shutdown(FALSE);
		return;
	}
...
/*
 * Read a string from file fp using IMCSTRALLOC
 * [Taken from Smaug's fread_string]
 */
const char *
imcfread_string(FILE * fp)
{
	char    buf[LSS];
	char   *plast;
	char    c;
	...
	do {
		if (feof(fp)) {
			imcbug("%s",
			    "imcfread_string: EOF encountered on read.");
			return IMCSTRALLOC("");
		}
		c = getc(fp);
	} while (isspace(c));
...
imcfread_number(FILE * fp)
{
	...
	do {
		if (feof(fp)) {
			imclog("%s",
			    "imcfread_number: EOF encountered on read.");
			return 0;
		}
		c = getc(fp);
	}
...
imcfread_letter(FILE * fp)
{
	...
	do {
		if (feof(fp)) {
			imclog("%s",
			    "imcfread_letter: EOF encountered on read.");
			return '\0';
		}
		c = getc(fp);
	}
...
imcfread_to_eol(FILE * fp)
{
	...
	do {
		if (feof(fp)) {
			imclog("%s",
			    "imcfread_to_eol: EOF encountered on read.");
			return;
		}
		c = getc(fp);
	}
...
imc_loadchar(CHAR_DATA * ch, FILE * fp, const char *word)
{
	...
	if (!strcasecmp(word, "IMCBlacklist")) {
		...
		IMCLINK(temp, FIRST_IMCBLACKLIST(ch), LAST_IMCBLACKLIST(ch),
		    next, prev);
		return TRUE;
	}
...
imcfread_config_file(FILE * fin)
{
	...
	for (;;) {
		...
		case '#':
			...
			break;
		case 'A':
			KEY("Autoconnect", this_imcmud->autoconnect,
			    imcfread_number(fin));
			KEY("AdminLevel", imc_minadminlevel,
			    imcfread_number(fin));
			break;
		case 'C':

			KEY("ClientPwd", this_imcmud->clientpw,
			    imcfread_string(fin));
			break;
		case 'D':
		...
		case 'H':
			...
			KEY("HubName", this_imcmud->hubname,
			    imcfread_string(fin));
			KEY("HubPort", this_imcmud->port, imcfread_number(fin));
			break;

		case 'I':
			KEY("InfoName", imc_siteinfo.name,
			    imcfread_string(fin));
			KEY("InfoHost", imc_siteinfo.host,
			    imcfread_string(fin));
			KEY("InfoPort", imc_siteinfo.port,
			    imcfread_number(fin));
			KEY("InfoEmail", imc_siteinfo.email,
			    imcfread_string(fin));
			KEY("InfoWWW", imc_siteinfo.www, imcfread_string(fin));
			KEY("InfoDetails", imc_siteinfo.details,
			    imcfread_string(fin));
			KEY("InfoBase", imc_siteinfo.base,
			    imcfread_string(fin));
			break;
		case 'L':
			...
			break;
		case 'M':
			KEY("MinPlayerLevel", imc_minplayerlevel,
			    imcfread_number(fin));
			KEY("MinImmLevel", imc_minimmlevel,
			    imcfread_number(fin));
			break;
		case 'N':
			KEY("NetworkName", imc_siteinfo.netname,
			    imcfread_string(fin));
		case 'S':
			KEY("ServerPwd", this_imcmud->serverpw,
			    imcfread_string(fin));
			break;
		case 'W':
...
imc_read_config(void)
{
	...
	for (;;) {
		...
		if (!strcasecmp(word, "IMCCONFIG") && this_imcmud == NULL) {
			...
			imcbug
			    ("imc_read_config: Bad section in config file: %s",
			    word);
			continue;
		}
	}
	...
	if (!this_imcmud) {
		imcbug("%s",
		    "imc_read_config: No hub connection information!!");
		imcbug("%s", "Network configuration aborted.");
		return FALSE;
	}

	if (!this_imcmud->hubname || !this_imcmud->host
	|| !this_imcmud->clientpw || !this_imcmud->serverpw
	|| !this_imcmud->port) {
		imcbug("%s",
		    "imc_read_config: Missing required configuration info.");
		imcbug("%s", "Network configuration aborted.");
		return FALSE;
	}
	...
	if (!imc_siteinfo.netname || imc_siteinfo.netname[0] == '\0') {
		imcbug("%s",
		    "imc_read_config: Missing NetworkName parameter in configuration file.");
		imcbug("%s", "Network configuration aborted.");
		return FALSE;
	}
...
imc_readblacklist(void)
{
	...
	while (!feof(inf) && !ferror(inf)) {
		...
		IMCLINK(entry, first_imc_mudblacklist, last_imc_mudblacklist,
		    next, prev);
	}
...
imc_loadhistory(void)
{
	...
	for (tempchan = first_imc_channel; tempchan; tempchan = tempchan->next) {
		...
		snprintf(filename, SSS, "%s/%s.hist", IMC_PATH,
		    tempchan->local_name);
		...
		for (x = 0; x < MAX_IMCHISTORY; x++) {
			if (feof(tempfile))
				...
				tempchan->history[x] =
				    IMCSTRALLOC(imcfread_line(tempfile));
		}
...
imc_startup(bool force)
{
	...
	/*
	 * Do not re-read configuration file on a network error reconnect.
	 * File reading is slow, and it will just be a waste of resources
	 */
	...
	if (!force) {
		imclog("%s",
		    "IMC2 data loaded. Autoconnect not set. IMC will need to"
...
imc_shutdown_network(void)
{
	...
	imclog("rx %ld packets, %ld bytes (%ld/second)", imc_stats.rx_pkts,
	    imc_stats.rx_bytes,
	    (imc_now ==
	    imc_stats.start) ? 0 : imc_stats.rx_bytes / (imc_now -
	    imc_stats.start));

	imclog("tx %ld packets, %ld bytes (%ld/second)", imc_stats.tx_pkts,
	    imc_stats.tx_bytes,
	    (imc_now ==
	    imc_stats.start) ? 0 : imc_stats.tx_bytes / (imc_now -
	    imc_stats.start));

	imclog("largest packet %d bytes", imc_stats.max_pkt);

	imclog("dropped %d packets by sequence number",
...
imc_shutdown(bool reconnect)
{
	...
	if (!this_imcmud || this_imcmud->state == CONN_NONE) {
		imcbug("%s",
		    "imc_shutdown: called without a hub connection up");
		return;
	}
	...
	for (entry = first_imc_mudblacklist; entry; entry = entry_next) {
		...
		IMCUNLINK(entry, first_imc_mudblacklist, last_imc_mudblacklist,
		    next, prev);
	...
	if (reconnect) {
		...
		imclog("%s",
		    "Connection to hub was lost. Reconnecting in approximately"
		    " 20 seconds.");
	}
...
int
imc_fill_fdsets(int maxfd, fd_set * iread, fd_set * iwrite, fd_set * exc)
{
	if (!this_imcmud || this_imcmud->state == CONN_NONE)
		return maxfd;
...
void
imc_idle_select(fd_set * iread, fd_set * iwrite, fd_set * exc, time_t now)
{
	char   *command;
	PACKET *p;
	int     error, error_len;
	...
	/*
	 * some data has been sent to the descriptor - insert it
	 * into the input buffer
	 */
	if (this_imcmud->state != CONN_NONE
	&& FD_ISSET(this_imcmud->desc, iread))
		do_imcread();

	/* process the input buffer */
	while (this_imcmud->state != CONN_NONE
	    && (command = imcgetline(this_imcmud->inbuf)) != NULL) {
		...
		case CONN_COMPLETE:
			...
			/*
			 * check for duplication, and register the packet
			...
			if (route
			    && (p->i.sequence + IMC_PACKET_LIFETIME) <
			    route->top_sequence) {
				imc_stats.sequence_drops++;
				break;
			}
			...
			/*
			 * Receive it if it's for us, otherwise it gets
			 * silently dropped
			 */
			if (!strcasecmp(imc_mudof(p->i.to), "*")
			|| !strcasecmp(imc_mudof(p->i.to), imc_name)) {
				/* strip the name from the 'to' */
				strlcpy(p->to, imc_nameof(p->i.to),
				    IMC_NAME_LENGTH);
	...
	/* Something could have caused shutdown during reading */
	if (this_imcmud->desc > 0) {
		...
		&& FD_ISSET(this_imcmud->desc, iwrite)) {
			...
			if (error) {
				imcbug("Coulnd't connect to host %s:%d, "
				    "reason: %s",
...
IMC_CHANNEL *
imc_findlchannel(const char *name)
{
	IMC_CHANNEL *c;
...
void
imc_recv_msg_r(const char *from, const char *realfrom, const char *chan,
    const char *txt, int emote)
{
	...
	if (!c->local_name || (c->policy != CHAN_PRIVATE
	    && c->policy != CHAN_CPRIVATE))
		return;

	/*
	 * We assume that anything redirected is automatically
	 * audible - since we trust the hub...
...
void
imc_recv_msg_b(const char *from, const char *chan, const char *txt, int emote)
{
	IMC_CHANNEL *c;
...
void
imc_recv_update(const char *from, const char *chan, const char *owner,
...
void
imc_recv_destroy(const char *from, const char *channel)
{
	IMC_CHANNEL *c;
	const char *mud;
...
void
ev_imc_firstrefresh(void *dummy)
{
	PACKET  out;
...
int
imc_recv_chan(PACKET * p, int bcast)
{
	/* redirected msg */
	if (!strcasecmp(p->type, "ice-msg-r")) {
		imc_recv_msg_r(p->from, imc_getkey(p, "realfrom", ""),
		    imc_getkey(p, "channel", ""), imc_getkey(p, "text", ""),
		    imc_getkeyi(p, "emote", 0));
		return 1;
	} else if (!strcasecmp(p->type, "ice-msg-b")) {
		imc_recv_msg_b(p->from, imc_getkey(p, "channel", ""),
		    imc_getkey(p, "text", ""), imc_getkeyi(p, "emote", 0));
		return 1;
	} else if (!strcasecmp(p->type, "ice-update")) {
		imc_recv_update(p->from, imc_getkey(p, "channel", ""),
		    imc_getkey(p, "owner", ""), imc_getkey(p, "operators", ""),
...
imc_readchannel(IMC_CHANNEL * channel, FILE * fp)
{
	...
	for (;;) {
		...
		case 'C':
			...
			KEY("ChanLocal", channel->local_name,
			    imcfread_string(fp));
			KEY("ChanRegF", channel->regformat,
			    imcfread_string(fp));
			KEY("ChanEmoF", channel->emoteformat,
			    imcfread_string(fp));
			KEY("ChanSocF", channel->socformat,
			    imcfread_string(fp));
			KEY("ChanLevel", channel->perm_level,
			    imcfread_number(fp));
			break;
...
imc_loadchannels(void)
{
	...
	for (;;) {
		...
		if (!strcasecmp(word, "IMCCHAN")) {
			...
			channel->refreshed = FALSE; /* Prevents crash trying  */
						    /* to use a bogus channel */
			IMCLINK(channel, first_imc_channel, last_imc_channel,
			    next, prev);
			imclog("configured %s as %s", channel->name,
			    channel->local_name);
			continue;
		} else if (!strcasecmp(word, "END"))
			break;
...
void
run_imccadmin(CHAR_DATA * ch, const char *argument)
{
...
void
run_imccsettings(CHAR_DATA * ch, const char *argument)
{
	...
	if (!channame || channame[0] == '\0' || !field || field[0] == '\0') {
		imc_to_char(color_itom(
		    ...
		    " emoteformat socformat\n"
		    "~cAll changes are saved on edit. More information on"
		    " each field may be obtained\n"
		    "~cby typing a field name without an accompanying value."
		    " Type \"~Gimccsettings\n"
		    "<channel name> show~c\" for a display of the current"
		    " values for a channel.\n"),
		    ch);
		return;
	}

	if ((channel = imc_findlchannel(channame)) == NULL
	    && (channel = imc_findchannel(channame)) == NULL) {
	...
	if (!strcasecmp(field, "show")) {
		...
		case CHAN_CLOSED:
			polly =
			    "~RClosed -- Sent to all muds but only invited"
			    " people can talk or listen";
			break;
		case CHAN_PRIVATE:
			polly =
			    "~yPrivate -- Only invited people can talk"
			    " or listen";
			break;
		case CHAN_CPRIVATE:
		...
		snprintf(buf, LSS,
		    ...
		    "~c  Excluded  ~W: ~C%s\n",
		    channel->name,
		    channel->local_name ? channel->local_name : "",
		    channel->regformat ? channel->regformat : "",
		    channel->emoteformat ? channel->emoteformat : "",
		    channel->socformat ? channel->socformat : "",
		    channel->perm_level ? channel->perm_level : 0, polly,
		    channel->owner, channel->operators, channel->invited,
		    channel->excluded);
	...
	if (!strcasecmp(field, "localize")) {
		...
		|| strlen(argument) > IMC_MNAME_LENGTH
		|| (strchr(argument, ' '))) {
			imc_to_char(color_itom
			    ("~cDescription: ~CLocalize sets the localname to"
			     " use the channel. It also\n"
			    "~C           ~Ccreates creates defaults for all"
			    " three format strings.\n"
			    "~cAccepted Values: ~CTakes any string without"
			    " spaces up to 20 characters.\n"),
			    ch);
			return;
		}
		...
		if (local) {
			char_printf(ch,
			    "Channel %s has already been localized as %s.\n",
	...
	if (!strcasecmp(field, "unlocalize")) {
		if (!argument || argument[0] == '\0'
		|| strcasecmp(argument, "now")) {
			imc_to_char(color_itom
			    ("~cDescription: ~CUnlocalize clears the "
			     "localname, the permission level,\n"
			    "~C             and all three format strings for"
			    " a channel.\n"
			    "~cAccepted Values: ~CTakes the value \"now\" to"
			    " confirm action.\n"),
			    ch);
			return;
		}
		...
		char_printf(ch,
		    "Channel %s has been unlocalized successfully.\n",
		    channel->name);
		imc_save_channels();
		return;
	}
	...
	if (!strcasecmp(field, "rename")) {
		...
		|| (strchr(argument, ' '))) {
			imc_to_char(color_itom
			    ("~cDescription: ~CRename modifies the localname"
		...
		char_printf(ch,
		    "Channel %s has had its local name successfully"
		    " changed to %s.\n",
		    channel->name, channel->local_name);
		imc_save_channels();
		return;
	...
	if (!strcasecmp(field, "permlevel")) {
		...
		if (!argument || argument[0] == '\0' || !is_number(argument)) {
			imc_to_char(color_itom
			    ("~cDescription: ~CPermlevel sets the minimum"
			     " permission level for using a\n"
			    "~C             localized channel.\n"
			    "~cAccepted Values: ~CTakes any integer between 1"
			    " and the admin permission level.\n"),
			    ch);
			return;
		}
		...
		if (value < 1 || value > PERM_ADMIN) {
			char_printf(ch,
			    "That value was outside of the range of 1 to %d.\n",
			    PERM_ADMIN);
			return;
		}
		...
		char_printf(ch,
		    "That channel has had its minimum permission level"
		    " changed to %d.\n", value);
		imc_save_channels();
		return;
	}
	...
	if (!strcasecmp(field, "regformat")) {
		if (!argument || argument[0] == '\0' || strlen(argument) > SSS) {
			imc_to_char(color_itom
			    ("~cDescription: ~CRegformat modifies the format"
			     " string for regular channel use.\n"
			    "~cAccepted Values: ~CTakes any string up to 256"
			    " characters with two %s's.\n"),
			    ch);
			return;
		}
		...
		if (!verify_format(argument, 2)) {
			imc_to_char
			    ("The regular format must contain two %s's.\n", ch);
			return;
		}
		...
		char_printf(ch,
		    "Channel %s has successfully had its regular format string"
		    " changed.\n", channel->name);
		imc_save_channels();
		return;
	}
	...
	if (!strcasecmp(field, "emoteformat")) {
		if (!argument || argument[0] == '\0' || strlen(argument) > SSS) {
			imc_to_char(color_itom(
			    "~cDescription: ~CEmoteformat modifies the format"
			    " string for emote channel use.\n"
			    "~cAccepted Values: ~CTakes any string up to 256"
			    " characters with two %s's.\n"),
			    ch);
			return;
		...
		char_printf(ch,
		    "Channel %s has successfully had its emote format string"
		    " changed.\n", channel->name);
		imc_save_channels();
		return;
	}
	...
	if (!strcasecmp(field, "socformat")) {
		if (!argument || argument[0] == '\0' || strlen(argument) > SSS) {
			imc_to_char(color_itom(
			    "~cDescription: ~CSocformat modifies the format"
			    " string for social channel use.\n"
			    "~cAccepted Values: ~CTakes any string up to 256"
			    " characters with one %s.\n"), ch);
			return;
		}
		...
		char_printf(ch,
		    "Channel %s has successfully had its social format string"
		    " changed.\n", channel->name);
		imc_save_channels();
		return;
	}
...
void
run_imcclist(CHAR_DATA * ch, const char *argument)
{
	char    buf[LSS];
	char   *polly;
	IMC_CHANNEL *c;
	...
	/*
	 * added level to the display for imcclist with no arguments for 2.00
	 * shogar - /1/26/2000
	 */
	...
	for (c = first_imc_channel; c; c = c->next) {
		...
		snprintf(buf + strlen(buf), LSS,
		    ...
		    c->local_name ? c->local_name : "~R(not local)  ", c->owner,
	...
	if (!allpriv) {
		imc_to_char(color_itom(
		     "\n~WType \"~Yimcclist all~W\" to see the entire listing,"
		     " including private\n"
...
imc_save_config(void)
{
	...
	fprintf(fp, "%s",
	    "# This is the IMC2 4.00 Continuum version of the IMC2"
	    " config file.\n");
	fprintf(fp, "%s",
	    "# When changing this information, be sure you don't remove"
	    " the tildes!\n");
	fprintf(fp, "%s",
	    "# This information can be edited online using the 'imcmsettings'"
	    " command.\n\n\n");
	...
	fprintf(fp, "%s",
	    "# This is the name of the IMC2 network that your mud will be"
	    " connected to.\n");
	...
	fprintf(fp, "%s",
	    "# This is the hostname or ip address of the hub that you will"
	    " connect to.\n");
	fprintf(fp, "HubAddr   %s~\n\n", this_imcmud->host);
	fprintf(fp, "%s",
	    "# This is the port on the host of the hub that you will"
	    " connect to.\n");
	fprintf(fp, "HubPort   %d\n\n", this_imcmud->port);
	fprintf(fp, "%s",
	    "# This is the password that you will use to confirm who you are"
	    " to the server.\n");
	fprintf(fp, "ClientPwd   %s~\n\n", this_imcmud->clientpw);
	fprintf(fp, "%s",
	    "# This is the password that the server will use to confirm who"
	    " it is to you.\n");
	fprintf(fp, "ServerPwd   %s~\n\n", this_imcmud->serverpw);
	fprintf(fp, "%s",
	    "# This sets whether your mud will attempt to connect to IMC"
	    " on startup.\n");
	fprintf(fp, "Autoconnect  %d\n\n", this_imcmud->autoconnect);
	fprintf(fp, "%s",
	    "# This option sets whether your mud will log incoming and"
	    " outgoing IMC packets.\n");
	fprintf(fp, "Debug        %d\n\n\n", imc_debug_on);
	...
	fprintf(fp, "%s",
	    "# Information about your mud that you might want other muds"
	    " to see goes here.\n\n\n");
	fprintf(fp, "%s",
	    "# This is the full name of your mud that you wish to display.\n");
	fprintf(fp, "InfoName     %s~\n\n", imc_siteinfo.name);
	fprintf(fp, "%s",
	    "# This is the hostname or ip address that you wish to have others"
	    " connect to.\n");
	fprintf(fp, "InfoHost     %s~\n\n", imc_siteinfo.host);
	fprintf(fp, "%s",
	    "# This is the port that you wish to have others connect"
	    " to for your mud.\n");
	fprintf(fp, "InfoPort     %d\n\n", imc_siteinfo.port);
	fprintf(fp, "%s",
	    "# This is the email address that you would like to be"
	    " contacted at.\n");
	fprintf(fp, "InfoEmail    %s~\n\n", imc_siteinfo.email);
	fprintf(fp, "%s",
	    "# This is the mud website that you would like interested parties"
	    " to go to.\n");
	...
	fprintf(fp, "%s",
	    "# This is a description of your mud that you would like other"
	    " muds to see.\n");
	...
	fprintf(fp, "%s",
	    "# 0 - Continuum Default 1 - Winter 2 - Fiery Bowels of Hell"
	    " 3 - IMC2 Classic\n");
...
imc_saveblacklist(void)
{
	...
	if (!(out = dfopen(IMC_PATH, IMC_BLACKLIST_FILE, "w"))) {
		imcbug("%s",
		    "imc_saveblacklist: error opening blacklist file for write");
		return;
	}
...
void
imc_send_emote(imc_char_data * from, int channel, char *argument, char *to)
{
	PACKET  out;
	...
	while (tobuf[0]) {
		if (!strcasecmp(tobuf, "*") || !strcasecmp(tobuf, imc_name)
...
void
imc_send_who(imc_char_data * from, const char *to, const char *type)
{
	PACKET  out;
...
void
imc_send_whois(imc_char_data * from, const char *to)
{
	PACKET  out;
...
void
imc_send_beep(imc_char_data * from, const char *to)
{
	PACKET  out;
...
void
run_imcptell(CHAR_DATA * ch, const char *argument)
{
	...
	if (IS_SET(IMC_PFLAGS(ch), PSET_IMCPTELL)) {
		imc_to_char(
		    "Enable incoming imcptells first ('imcpsettings +imcptell').\n",
		    ch);
		return;
	}
...
void
run_imcpreply(CHAR_DATA * ch, const char *argument)
{
	char    buf[LSS + 30];
...
void
run_imcpbeep(CHAR_DATA * ch, const char *argument)
{
	char    buf[LSS];
	imc_char_data *chdata = imc_getdata(ch);
	...
	if (IS_SET(IMC_PFLAGS(ch), PSET_IMCPBEEP)) {
		imc_to_char(
		    "Enable incoming imcpbeep first ('imcpsettings +imcpbeep').\n",
		    ch);
		return;
	}
...
void
run_imcpafk(CHAR_DATA * ch, const char *argument)
{
	CHECKIMC(ch);
...
void
run_imcplist(CHAR_DATA * ch, const char *argument)
{
	imc_char_data *chdata = imc_getdata(ch);
...
void
run_imcpfind(CHAR_DATA * ch, const char *argument)
{
	imc_char_data *chdata = imc_getdata(ch);
...
void
run_imcpinfo(CHAR_DATA * ch, const char *argument)
{
	char    name[LSS];
	imc_char_data *chdata = imc_getdata(ch);
...
void
run_imcminfo(CHAR_DATA * ch, const char *argument)
{
	char    arg[LSS];
	imc_char_data *chdata = imc_getdata(ch);
	...
	CHECKMUD(ch, arg);
	imc_send_who(chdata, arg, (argument
	    && argument[0] != '\0' ? argument : "help"));
	IMCDISPOSE(chdata);
}

void
run_imcmlist(CHAR_DATA * ch, const char *argument)
{
	CHECKIMC(ch);
...
void
run_imcmsettings(CHAR_DATA * ch, const char *argument)
{
	char    arg1[MSS];
	...
	if (!argument || argument[0] == '\0') {
		imc_to_char(color_itom(
		    ...
		    "~C  infoname infohost infoport infoemail infowww infobase"
		    " infodetails\n"
		    "~G--Miscellaneous Fields\n" "~C  whostyle\n"
		    "~cAll changes are saved on edit. More information on each"
		    " field may be obtained\n"
		    "~cby typing a field name without an accompanying value."
		    " Type \"~Gimcmsettings show~c\"\n"
		    "~cfor a display of the current values for all available"
		    " fields.\n"), ch);
		return;
	}
	...
	if (!strcasecmp(argument, "show")) {
		...
		char_printf(ch, color_itom("~cConnected      ~W: ~C%s\n"),
		    this_imcmud->state == CONN_COMPLETE ? "Yes" : "No");
	...
	if (!strcasecmp(arg1, "connected")) {
		if (!argument || argument[0] == '\0') {
			imc_to_char(color_itom(
			    "~cDescription: ~CConnected either connects or"
			     " disconnects the IMC client\n"
			    "             ~Cfrom the hub. Also, it can stop"
			    " the reconnect process\n"
			    "             ~Cwith \"no\".\n"
			    "~cAccepted Values: ~CTakes the values \"yes\" "
			    "and \"no\".\n"), ch);
			return;
		}
		...
		if (!strcasecmp(argument, "yes")) {
			if (this_imcmud->state > CONN_NONE) {
				imc_to_char(
				    "The IMC client is already connected"
				    " to the hub.\n", ch);
				return;
			}

			imc_to_char(
			    "The IMC client will now connect itself"
			    " to the hub.\n", ch);
			imcwait = 0;
			imc_startup(TRUE);
			return;
		...
		if (!strcasecmp(argument, "no")) {
			if (imcwait > 0) {
				imc_to_char(
				    "The IMC client will now stop its"
				    " reconnect process to the hub.\n", ch);
				imcwait = 0;
				return;
			}
			...
			if (this_imcmud->state < CONN_COMPLETE) {
				imc_to_char(
				    "The IMC client is already disconnected"
				    " from the hub.\n", ch);
				return;
			}

			imc_to_char(
			    "The IMC client will now disconnect itself from"
			    " the hub.\n", ch);
			imc_shutdown(FALSE);
			return;
		}
	...
	if (!strcasecmp(arg1, "imcname")) {
		...
		|| (strchr(argument, ' '))) {
			imc_to_char(color_itom(
			    "~cDescription: ~CImcname is the name that the"
			    " IMC2 network will know\n"
			    "             ~Cthis mud as.\n"
			    "~cAccepted Values: ~CTakes any string without"
			    " spaces up to 20 characters.\n"), ch);
			return;
		...
		if (this_imcmud->state > CONN_NONE) {
			imc_to_char(
			    "Imcname can not be modified while connected.\n",
			    ch);
			return;
		}
		...
		char_printf(ch,
		    "This mud will now be known to the network as %s\n",
		    argument);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "hubname")) {
		...
		|| (strchr(argument, ' '))) {
			imc_to_char(color_itom
			    ("~cDescription: ~CHubname sets the name that"
			     " IMC will call the hub.\n"
			    "~cAccepted Values: ~CTakes any string without"
			    " spaces up to 20 characters.\n"),
			    ch);
			return;
		...
		if (this_imcmud->state > CONN_NONE) {
			imc_to_char(
			    "Hubname can not be modified while connected.\n",
			    ch);
			return;
		}
	...
	if (!strcasecmp(arg1, "hubaddr")) {
		...
		|| (strchr(argument, ' '))) {
			imc_to_char(color_itom(
			    "~cDescription: ~CHubaddr sets the hostname or"
			    " ip address that IMC\n"
			    "             ~Cwill use to try to connect to"
			    " the hub.\n"
			    "~cAccepted Values: ~CTakes any string without"
		...
		if (this_imcmud->state > CONN_NONE) {
			imc_to_char(
			    "Hubaddr can not be modified while connected.\n",
			    ch);
			return;
		}
	...
	if (!strcasecmp(arg1, "hubport")) {
		...
		if (!argument || argument[0] == '\0' || !is_number(argument)) {
			imc_to_char(color_itom(
			    "~cDescription: ~CHubport sets the port that IMC"
			    " will try to connect\n"
			    "             ~Cto the hub at.\n"
			    "~cAccepted Values: ~CTakes any integer between 1"
			    " and 65,535.\n"), ch);
		...
		if (this_imcmud->state > CONN_NONE) {
			imc_to_char(
			    "Hubport can not be modified while connected.\n",
			    ch);
			return;
		}
		...
		if (value < 1 || value > 65535) {
			imc_to_char(
			    "That value was outside of the range"
			    " of 1 to 65,535.\n", ch);
			return;
		}
	...
	if (!strcasecmp(arg1, "clientpwd")) {
		if (!argument || argument[0] == '\0'
		|| strlen(argument) > IMC_PW_LENGTH
		|| (strchr(argument, ' '))) {
			imc_to_char(color_itom(
			    "~cDescription: ~CClientpwd is the password that"
			    " the IMC client uses to\n"
		...
		if (this_imcmud->state > CONN_NONE) {
			imc_to_char(
			    "Clientpwd can not be modified while connected.\n",
			    ch);
			return;
		}
		...
		char_printf(ch,
		    "The client will authenticate itself to the hub with %s.\n",
		    argument);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "serverpwd")) {
		...
		|| (strchr(argument, ' '))) {
			imc_to_char(color_itom(
			    "~cDescription: ~CServerpwd is the password that"
			    " the IMC hub uses to\n "
		...
		if (this_imcmud->state > CONN_NONE) {
			imc_to_char(
			    "Serverpwd can not be modified while connected.\n",
			    ch);
			return;
		}
		...
		char_printf(ch,
		    "The hub will authenticate itself to the client with %s.\n",
		    argument);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "autoconnect")) {
		if (!argument || argument[0] == '\0') {
			imc_to_char(color_itom(
			    "~cDescription: ~CAutoconnect sets whether or not"
			    " IMC will start at mud boot.\n"
			    "~cAccepted Values: ~CTakes the values \"yes\""
			    " and \"no\".\n"), ch);
			return;
		}
	...
	if (!strcasecmp(arg1, "debug")) {
		if (!argument || argument[0] == '\0') {
			imc_to_char(color_itom(
			    "~cDescription: ~CDebug sets whether or not all"
	...
	if (!strcasecmp(arg1, "netname")) {
		if (!argument || argument[0] == '\0' || strlen(argument) > 10
		    || (strchr(argument, ' '))) {
			imc_to_char(color_itom(
			    "~cDescription: ~CNetname is the name of the"
			    " IMC2 network that this mud\n"
			    "             ~Cis connected to. The name must"
			    " be the exact same as the\n"
			    "             ~Chub in order to connect to"
			    " a IMC2 3.11 Hermes or later hub.\n"
			    "~cAccepted Values: ~CTakes any string without"
			    " spaces up to 10 characters.\n"), ch);
			return;
		}

		if (this_imcmud->state > CONN_NONE) {
			imc_to_char(
			    "Netname can not be modified while connected.\n",
			    ch);
			return;
		}
		...
		char_printf(ch,
		    "The IMC2 network that this mud is connected to will"
		    " now be known as %s\n", argument);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "minplayerlevel")) {
		...
		if (!argument || argument[0] == '\0' || !is_number(argument)) {
			imc_to_char(color_itom(
			    "~cDescription: ~CMinplayerlevel sets the minimum"
			    " level for a player to use IMC.\n"
			    "~cAccepted Values: ~CTakes any integer between 1"
			    " and the mud's maximum level.\n"), ch);
			return;
		}
		...
		char_printf(ch,
		    "The minimum player access level has been set to %d.\n",
		    value);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "minimmlevel")) {
		...
		if (!argument || argument[0] == '\0' || !is_number(argument)) {
			imc_to_char(color_itom(
			    "~cDescription: ~CMinimmlevel sets the minimum"
			    " level for someone to have\n"
		...
		char_printf(ch,
		    "The minimum immortal privilege level has been set to %d.\n",
		    value);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "minadminlevel")) {
		...
		if (!argument || argument[0] == '\0' || !is_number(argument)) {
			imc_to_char(color_itom(
			    "~cDescription: ~CMinadminlevel sets the minimum"
			    " level for someone to have\n"
		...
		char_printf(ch,
		    "The minimum administrative privilege level has been set to %d.\n",
		    value);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "infoname")) {
		if (!argument || argument[0] == '\0' || strlen(argument) > 35) {
			imc_to_char(color_itom(
			    "~cDescription: ~CInfoname is what information"
			    " services such as imcplist\n"
			    "             ~Cand imcminfo will display as this"
			    " mud's name.\n"
			    "~cAccepted Values: ~CTakes any string up to 35"
			    " characters.\n"), ch);
			return;
		}
	...
	if (!strcasecmp(arg1, "infohost")) {
		...
		|| (strchr(argument, ' '))) {
			imc_to_char(color_itom(
			    "~cDescription: ~CInfohost is what information"
			    " services such as imcplist\n"
			    "             ~Cand imcminfo will display as this"
			    " mud's host.\n"
			    "~cAccepted Values: ~CTakes any string without"
		...
		char_printf(ch,
		    "This mud will now be displayed as being at %s.\n",
		    argument);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "infoport")) {
		...
		if (!argument || argument[0] == '\0' || !is_number(argument)) {
			imc_to_char(color_itom(
			    "~cDescription: ~CInfoport is what information"
			    " services such as imcplist\n"
			    "             ~Cand imcminfo will display as this"
			    " mud's port.\n"
			    "~cAccepted Values: ~CTakes any integer between 1"
		...
		if (value < 1 || value > 65535) {
			imc_to_char
			    ("That value was outside of the range of 1 to 65,535.\n",
			    ch);
			return;
		}
		...
		char_printf(ch,
		    "This mud will now be displayed as being on port %d.\n",
		    value);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "infoemail")) {
		if (!argument || argument[0] == '\0'
		|| strlen(argument) > SSS + 24) {
			imc_to_char(color_itom(
			    "~cDescription: ~CInfoemail is what information"
			    " services such as imcplist\n"
			    "             ~Cand imcminfo will display as this"
			    " mud's administrative email.\n"
			    "~cAccepted Values: ~CTakes any string up to 280"
			    " characters.\n"), ch);
			return;
		}
		...
		char_printf(ch,
		    "This mud's administrative email will now be displayed as %s.\n",
		    argument);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "infowww")) {
		...
		|| (strchr(argument, ' '))) {
			imc_to_char(color_itom(
			    "~cDescription: ~CInfowww is what information"
			    " services such as imcplist\n"
			    "             ~Cand imcminfo will display as this"
			    " mud's web address.\n"
			    "~cAccepted Values: ~CTakes any string without"
		...
		char_printf(ch,
		    "This mud's webpage will now be displayed as being at %s.\n",
		    argument);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "infobase")) {
		...
		if (!argument || argument[0] == '\0' || strlen(argument) > 20) {
			imc_to_char(color_itom(
			    ...
			    "             ~Cand imcminfo will display as this"
			    " mud's codebase.\n"
			    "~cAccepted Values: ~CTakes any string up to 20"
			    " characters.\n"), ch);
			return;
		}
		...
		char_printf(ch,
		    "This mud's codebase will now be displayed as %s.\n",
		    argument);
		imc_save_config();
		...
		snprintf(cbase, MSS, "%s %s", IMC_VERSION_ID,
		    imc_siteinfo.base);
	...
	if (!strcasecmp(arg1, "infodetails")) {
		if (!argument || argument[0] == '\0' || strlen(argument) > SSS) {
			imc_to_char(color_itom(
			    "~cDescription: ~CInfodetails is what information"
			    " services such as imcplist\n"
			    "             ~Cand imcminfo will display as"
			    " the mud's short description.\n"
			    "~cAccepted Values: ~CTakes any string up to 256"
			    " characters.\n"), ch);
			return;
		}
		...
		char_printf(ch,
		    "This mud's short description will now be \"%s\".\n",
		    argument);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "whostyle")) {
		...
		if (!argument || argument[0] == '\0' || strlen(argument) > 25) {
			imc_to_char(color_itom(
			    "~cDescription: ~CWhostyle sets the color pattern"
			    " used for your imcplist.\n"
			    "~cAccepted Values: ~CTakes any integer between 0"
			    " and the mud's maximum whostyle\n"
			    "                 ~Cor any string up to 25"
			    " characters for the whostyle's name.\n"
			    "~cCurrent WhoStyles:\n"), ch);
		...
		if (!is_number(argument)) {
			for (value = 0; value < MAX_WHOSTYLE; value++)
				if (!imc_str_prefix(whostylename_table[value],
				    argument))
					break;
		} else
			value = atoi(argument);
		...
		if (value < 0 || value >= MAX_WHOSTYLE) {
			char_printf(ch,
			    "That value was outside of the range of 0 to %d.\n",
			    MAX_WHOSTYLE - 1);
			return;
		}
		...
		imc_whostyle = value;
		char_printf(ch,
		    "The color pattern style for your imcplist has been"
...
void
run_imcmblacklist(CHAR_DATA * ch, const char *argument)
{
	int     count;
	IMC_BLACKLIST *entry;
	char    arg[MSS];
	...
	if (!argument || argument[0] == '\0') {
		imc_to_char(color_itom(
		    ...
		    "~c        imcmblacklist remove <mud> - ~Cremoves a"
		    " mud from the blacklist\n"), ch);
		return;
	}
	...
	if (!strcasecmp(arg, "list")) {
		...
		for (count = 0, entry = first_imc_mudblacklist; entry;
...
void
run_imcmstat(CHAR_DATA * ch, const char *argument)
{
	CHECKIMC(ch);
...
void
run_imcpsettings(CHAR_DATA * ch, const char *argument)
{
	char    arg[MSS];
	bool    toggle;
	unsigned char mode;
	...
	if (!argument || argument[0] == '\0') {
		imc_to_char(color_itom
		    ...
		    "~c        imcpsettings subscribe -channel - ~Cdeletes"
		    " a subscription to a channel\n"
		    "~c        imcpsettings blacklist +mud - ~Cadds a mud"
		    " to the blacklist\n"
		    "~c        imcpsettings blacklist -mud - ~Cremoves a mud"
		    " from the blacklist\n"
		    "~c        imcpsettings blacklist +person@mud - ~Cadds a"
		    " player to the blacklist\n"
		    "~c        imcpsettings blacklist -person@mud - ~Cremoves"
		    " a player from the blacklist\n"
		    "~c        imcpsettings function +nfunction  - ~Cswitches"
		    " on a network function\n"
		    "~c        imcpsettings function -nfunction  - ~Cswitches"
		    " off a network function\n"
		...
		imc_to_char(color_itom(
		    ...
		    "There is also the 'all' option to set or unset all"
		    " available functions.\n"), ch);
	...
	if (!strcasecmp(arg, "show")) {
		...
		for (i = 0; i < numfunctions; i++) {
			if (!function_usable(ch, imc_functions[i].deny_flag,
			    imc_functions[i].allow_flag,
			    imc_functions[i].minlevel)) {
				snprintf(dfunc + strlen(dfunc),
				    MSS - strlen(dfunc), " %s",
				    imc_functions[i].name);
				j++;
			} else if (IS_SET(IMC_PFLAGS(ch),
			    imc_functions[i].pset_flag)) {
				snprintf(nfunc + strlen(nfunc),
				    MSS - strlen(nfunc), " %s",
				    imc_functions[i].name);
				k++;
			} else {
				snprintf(afunc + strlen(afunc),
				    MSS - strlen(afunc), " %s",
				    imc_functions[i].name);
				l++;
			}
		...
		char_printf(ch, color_itom("~cChannel Subscriptions: ~C%s\n"),
		    (IMC_CSUBSCRIBED(ch)
		    && IMC_CSUBSCRIBED(ch)[0] !=
		    '\0' ? IMC_CSUBSCRIBED(ch) : ""));

		imc_to_char(color_itom("~cBlacklisted Muds:~C"), ch);

		if (FIRST_IMCBLACKLIST(ch) != NULL
		&& LAST_IMCBLACKLIST(ch) != NULL)
			for (entry = FIRST_IMCBLACKLIST(ch); entry;
			    entry = entry->next)
				char_printf(ch, color_itom("~C %s"),
				    entry->name);
	...
	while ((argument = imc_getarg(argument, arg, MSS)), arg[0]) {
		...
		if (mode == 1) {	/* Channel Subscriptions */
			if (imc_hasname(IMC_CSUBSCRIBED(ch), arg + 1)) {
				...
				imc_to_char(
				    "No longer subscribed to channel.\n", ch);
			} else {
				if (!imc_findlchannel(arg + 1)) {
					imc_to_char(
					    "No such channel localized.\n",
					    ch);
				} else {
					if (!toggle) {
						imc_to_char(
					...
					imc_addname(&IMC_CSUBSCRIBED(ch),
					    arg + 1);
			...
			if (entry) {
				...
				IMCUNLINK(entry, FIRST_IMCBLACKLIST(ch),
				    LAST_IMCBLACKLIST(ch), next, prev);

				IMCDISPOSE(entry);

				if ((strchr(arg + 1, '@')))
					imc_to_char(
					    "That person has been removed from"
					    " your blacklist.\n", ch);
				else
					imc_to_char(
					    "That mud has been removed from"
					    " your blacklist.\n", ch);
			} else {
				if (!toggle) {
					if ((strchr(arg + 1, '@')))
						imc_to_char(
						    "That person is not on"
						    " your blacklist.\n", ch);
					else
						imc_to_char(
						    "That mud is not on your"
						    " blacklist.\n", ch);
					return;
				}
				...
				IMCLINK(entry, FIRST_IMCBLACKLIST(ch),
				    LAST_IMCBLACKLIST(ch), next, prev);

				if ((strchr(arg + 1, '@')))
					imc_to_char(
					    "That person has been added to"
					    " your blacklist.\n", ch);
				else
					imc_to_char(
					    "That mud has been added to your"
					    " blacklist.\n", ch);
			}
			...
			if (!strcasecmp(arg + 1, "all")) {
				if (toggle) {
					imc_to_char(
					    "ALL available IMC network"
					     " functions are now on.\n", ch);

					for (i = 0; i < numfunctions; i++) {
						if (function_usable(ch,
						    imc_functions[i].deny_flag,
						    imc_functions[i].allow_flag,
						    imc_functions[i].minlevel)
						    && imc_functions[i].
						    pset_flag !=
						    PSET_NOTSUPPORTED)
							REMOVE_BIT(IMC_PFLAGS
							    (ch),
							    imc_functions[i].
							    pset_flag);
					}
				...
				if (i == numfunctions) {
					char_printf(ch,
					    "You don't have access to an IMC "
					    "network function called \"%s\".\n",
					    arg + 1);
					continue;
				}
			}
			...
			if (imc_functions[i].pset_flag == PSET_NOTSUPPORTED) {
				char_printf(ch,
				    "%s can not have its status changed.\n",
				    imc_functions[i].name);
				continue;
			}

			if (toggle
			&& !IS_SET(IMC_PFLAGS(ch),
			    imc_functions[i].pset_flag)) {
...
void
run_imcpflags(CHAR_DATA * ch, const char *argument)
{
	char    arg[MSS];
	CHAR_DATA *victim = NULL;
	DESCRIPTOR_DATA *d;
	...
	if (!arg || arg[0] == '\0') {
		imc_to_char(color_itom(
		    ...
		    "        ~cimcpflags <char> -<network function> - ~Cset"
		    " deny flag\n"
		    "        ~cimcpflags <char> =<network function> - ~Creset"
		    " allow/deny flag\n"), ch);
		return;
	}
	...
	if (!argument || argument[0] == '\0') {
		...
		for (i = 0; i < numfunctions; i++) {
			...
			snprintf(funcstring, SSS, "~c%16s ~W- %s\n",
			    imc_functions[i].name, IS_SET(IMC_PFLAGS(victim),
			    imc_functions[i].
			    allow_flag) ? "~CAllow Flag~c" :
			    IS_SET(IMC_PFLAGS(victim),
			    imc_functions[i].
			    allow_flag) ? "~CDeny Flag~c" : "~CNo Flags");
	...
	case 0:		/* set deny flag */
		...
		if (get_permlevel(victim) < imc_functions[i].minlevel) {
			imc_to_char(
			    "That player is already below this network"
			    " function's permission level.\n", ch);
			return;
		}

		if (imc_functions[i].deny_flag == DENY_NOTSUPPORTED) {
			imc_to_char(
			    "That network function can not be denied.\n", ch);
			return;
		}
		...
		char_printf(victim,
		    "The gods have revoked your %s privileges.\n",
		    imc_functions[i].name);
	...
	case 1:		/* set allow flag */
		if (get_permlevel(victim) >= imc_functions[i].minlevel) {
			imc_to_char(
			    "That player is already at or above this network"
			    " function's permission level.\n", ch);
			return;
		}
		...
		if (imc_functions[i].allow_flag == ALLOW_NOTSUPPORTED) {
			imc_to_char(
			    "That network function can not be specially"
			    " allowed.\n", ch);
			return;
		}
	...
	case 2:		/* clears flags */
		if (IS_SET(IMC_PFLAGS(victim), imc_functions[i].allow_flag)) {
			REMOVE_BIT(IMC_PFLAGS(victim),
			    imc_functions[i].allow_flag);
			char_printf(victim,
			    "The gods have removed your %s privileges.\n",
			    imc_functions[i].name);
			imc_to_char("Allow flag cleared.\n", ch);
		} else if (IS_SET(IMC_PFLAGS(victim),
		    imc_functions[i].deny_flag)) {
			REMOVE_BIT(IMC_PFLAGS(victim),
			    imc_functions[i].deny_flag);
			char_printf(victim,
			    "The gods have restored your %s privileges.\n",
			    imc_functions[i].name);
 ...
 */
void
run_imcmfind(CHAR_DATA * ch, const char *argument)
{
	CHECKIMC(ch);
...
/* Mud Directed Commands */
	...
	{ "imcminfo", run_imcminfo, PERM_PLAYER },
	{ "imcquery", run_imcminfo, PERM_PLAYER },
	{ "rquery", run_imcminfo, PERM_PLAYER },
	/* Lists all currently and recently connected muds */
	{ "imcmlist", run_imcmlist, PERM_PLAYER },
	{ "imclist", run_imcmlist, PERM_PLAYER },
...
/* Player Directed Commands */
	...
	{ "imcpbeep", run_imcpbeep, PERM_PLAYER },
	{ "imcbeep", run_imcpbeep, PERM_PLAYER },
	{ "rbeep", run_imcpbeep, PERM_PLAYER },
	/* Searches for all visible players with a given name */
	{ "imcpfind", run_imcpfind, PERM_PLAYER },
	{ "imclocate", run_imcpfind, PERM_PLAYER },
	{ "rwhois", run_imcpfind, PERM_PLAYER },
	/* Retrieves all available information on a player */
	{ "imcpinfo", run_imcpinfo, PERM_PLAYER },
	{ "imcfinger", run_imcpinfo, PERM_PLAYER },
	{ "rfinger", run_imcpinfo, PERM_PLAYER },
	/* Lists all visible players on a mud */
	{ "imcplist", run_imcplist, PERM_PLAYER },
	{ "imcwho", run_imcplist, PERM_PLAYER },
	{ "rwho", run_imcplist, PERM_PLAYER },
	/* Continues a private communication with a player */
	{ "imcpreply", run_imcpreply, PERM_PLAYER },
	{ "imcreply", run_imcpreply, PERM_PLAYER },
	{ "rreply", run_imcpreply, PERM_PLAYER },
	/*
	 * Modifies the general IMC settings of a player,
	 * along with the player's channel settings
	 */
	{ "imcpsettings", run_imcpsettings, PERM_PLAYER },
	{ "imcflags", run_imcpsettings, PERM_PLAYER },
	{ "imclisten", run_imcpsettings, PERM_PLAYER },
	{ "rchannels", run_imcpsettings, PERM_PLAYER },
	{ "ichan", run_imcpsettings, PERM_PLAYER },
	/* Sends a private communication to a player */
	{ "imcptell", run_imcptell, PERM_PLAYER },
	{ "imctell", run_imcptell, PERM_PLAYER },
	{ "rtell", run_imcptell, PERM_PLAYER },
...
/* Mud Directed Commands */
	...
	{ "imcmfind", run_imcmfind, PERM_IMMORTAL },
	{ "imcping", run_imcmfind, PERM_IMMORTAL },
	{ "rping", run_imcmfind, PERM_IMMORTAL },
	/* Retrieves general and network information about the mud */
	{ "imcmstat", run_imcmstat, PERM_IMMORTAL },
	{ "imcstats", run_imcmstat, PERM_IMMORTAL },
	{ "istats", run_imcmstat, PERM_IMMORTAL },
...
/* Channel Directed Commands */
	...
	{ "imccadmin", run_imccadmin, PERM_ADMIN },
	{ "imccommand", run_imccadmin, PERM_ADMIN },
	{ "icommand", run_imccadmin, PERM_ADMIN },
	/* Modifies a channel's mud specific settings */
	{ "imccsettings", run_imccsettings, PERM_ADMIN },
	{ "imcsetup", run_imccsettings, PERM_ADMIN },
	{ "isetup", run_imccsettings, PERM_ADMIN },

/* Mud Directed Commands */

	/* Modifies a list of hostile muds to be blacklisted */
	{ "imcmblacklist", run_imcmblacklist, PERM_ADMIN },
	{ "imcignore", run_imcmblacklist, PERM_ADMIN },
	{ "rignore", run_imcmblacklist, PERM_ADMIN },
	/* Modifies the general IMC settings for the mud */
	{ "imcmsettings", run_imcmsettings, PERM_ADMIN },
	{ "imc", run_imcmsettings, PERM_ADMIN },

/* Player Directed Commands */

	/* Modifies the privileges of a player */
	{ "imcpflags", run_imcpflags, PERM_ADMIN },
	{ "imcdeny", run_imcpflags, PERM_ADMIN },
	{ "rchanset", run_imcpflags, PERM_ADMIN },
	
	{NULL, NULL, 0}
};

/*
 * check for IMC channels, return TRUE to stop command processing,
 * FALSE otherwise
 */
bool
imc_command_hook(CHAR_DATA * ch, const char *command, const char *argument)
{
	...
	if (!this_imcmud) {
		imcbug("%s",
		    "Ooops. IMC being called without configuration loaded.");
		return FALSE;
	}
...
const char *
return_imcsocial(const char *socialname, unsigned char type)
{
#ifdef SOCIALSTRUCT
	social_t *social;
#else
...
const char *
imc_act_string(const char *format, CHAR_DATA * ch, const char *vname)
{
	...
	while (*format != '\0') {
		...
		if ((!vname || vname[0] == '\0')
			...
			default:
				...
				break;
			case 'n':
				i = imc_makename(CH_IMCNAME(ch), imc_name);
				break;
			case 'N':
				i = vname;
				break;
			case 'e':
				i = he_she[URANGE(0, CH_IMCSEX(ch), 2)];
				break;
			case 'E':
				i = "it";
				break;
			case 'm':
				i = him_her[URANGE(0, CH_IMCSEX(ch), 2)];
				break;
			case 'M':
				i = "it";
				break;
			case 's':
				i = his_her[URANGE(0, CH_IMCSEX(ch), 2)];
				break;
			case 'S':
				i = "its";
				break;
			case 'k':
				imc_getarg(CH_IMCNAME(ch), tmp_str, LSS);
				i = (char *)tmp_str;
				break;
			case 'K':
				imc_getarg(vname, tmp_str, LSS);
				i = (char *)tmp_str;
				break;
			}
		}
...
bool
imc_channel_handler(CHAR_DATA * ch, const char *command, const char *argument)
{
	IMC_CHANNEL *c;
	DESCRIPTOR_DATA *d;
	...
	/* Social code redone by Rogel, based off of work performed by Senir */
		...
		if (vname && vname[0] != '\0') {
			...
			if (strcasecmp(vname, "Self")
			&& (strcasecmp(CH_IMCNAME(ch), imc_nameof(vname))
			|| (separator
			    && strcasecmp(imc_mudof(vname), imc_name)))) {
				if (!separator) {
					imc_to_char(
					    "The social target must be in the"
					    " form of person@mud.\n", ch);
					return TRUE;
				}
		...
		|| return_imcsocial(socialname, type)[0] == '\0') {
			...
			char_printf(ch,
			    "%s is either not a social or that part of"
			    " it is blank.\n", socialname);
			return TRUE;
		}

		strlcpy(newstring, imc_act_string(return_imcsocial(socialname,
		    type), ch, vname), LSS);
	...
	/*
	 * Local echo - bear in mind this doesn't mean your message
	 * actually got out!
	 * This is a punt, nothing more.
	 */
	if (emote < 2) {
		snprintf(mbuf, LSS, emote ? c->emoteformat : c->regformat,
		    imc_makename(CH_IMCNAME(ch), imc_name), argument);
	} else
		snprintf(mbuf, LSS, c->socformat, argument);
	for (d = first_descriptor; d; d = d->next) {
		if (d->character
		&& imc_hasname(IMC_CSUBSCRIBED(d->character), c->local_name)
