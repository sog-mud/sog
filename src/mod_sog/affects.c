/*
 * $Id: affects.c,v 1.30 2000-01-19 06:51:46 fjoe Exp $
 */
...
void affect_modify(CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd)
{
	...
	default:
		if (IS_NPC(ch)) {
			...
			log("affect_modify: %s: unknown location %d where: %d",
...
void affect_modify_room(ROOM_INDEX_DATA *room, AFFECT_DATA *paf, bool fAdd)
{
	...
	{
	default:
		bug("affect_modify_room: unknown location %d",
		    INT(paf->location));
		return;
	...
	if (!room->affected) {
		...
		if (top_affected_room  == room)
			...
			for(prev = top_affected_room;
				...
				prev = prev->aff_next);
			if (prev == NULL) {
				bug("affect_remove_room: cannot find room");
				return;
			}
