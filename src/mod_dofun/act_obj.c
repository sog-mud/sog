/*
 * $Id: act_obj.c,v 1.6 1998-04-29 11:27:21 efdi Exp $
 */
...
void do_second_wield (CHAR_DATA *ch, char *argument)
{
    OBJ_DATA *obj;
    char buf[MAX_STRING_LENGTH]; 
    int sn,skill;
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < skill_table[gsn_second_weapon].skill_level[ch->class] )
    {
	send_to_char(
	    "You don't know how to wield a second weapon.\n\r", ch );
	return;
    }
