/*
 * $Id: act_info.c,v 1.271.2.85 2004-02-24 10:14:59 fjoe Exp $
 */
...
void do_practice(CHAR_DATA *ch, const char *argument)
{
	...
	if ((cl = class_lookup(ch->class)) == NULL) {
		printlog("do_practice: %s: class %d: unknown",
			   ch->name, ch->class);
		return;
	}
	      ...
	      const char *list, const char *name_list)
{
	...
	for (; name[0]; list = first_arg(list, name, sizeof(name), FALSE)) {
		...
		buf_printf(output, "%-8s  %3d  %-5s  %-3s  %7s-%-7s %s%s{x\n",
...
void do_compress(CHAR_DATA *ch, const char *argument)
{
	...
	case 1:
		support = "mccp version 1 is supported by client";
		break;
	case 2:
		support = "mccp version 2 is supported by client";
		break;
	default:
		support = "mccp is not supported by client";
		break;
	}
...
" {CReligion  :{x unknown                   {CClan     :{x unknown\n");
	...
	||  IS_NULLSTR(PC(victim)->ll_host))
		...
		if (loaded) {
			buf_printf(output, " {CLast login:{x %s from %s (%s)\n",
